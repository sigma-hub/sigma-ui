---
title: AutoForm
description: Component that auto generates a form from Zod schema.
...
This example needs the following components:

```bash
npx sigma-ui add auto-form form button popover switch checkbox radio-group input textarea label select calendar separator
```
...
```bash
npx sigma-ui add form auto-form
```
